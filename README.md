...
Vanilla's post-break hit delay keeps the next ore or route obstacle locked without consuming destruction attempts or registering a false completion until block breaking actually starts.
...
The release artifact is `build/libs/voris-hub-1.10.114.jar`. `verifyRelease` runs unit tests and checks final JAR metadata, required classes, and Java 8 bytecode.
