...
- `flight`: WWE-style Static and Vanilla movement modes with independent horizontal/ascent and descent speeds, current-tick input ownership, and guarded landing behavior. Legacy Hypixel configs migrate to Static.
...
`meleeAura` measures its range from the player's eyes to the nearest point of a target's hitbox. Its default is `3.0` blocks and its configurable maximum is `6.0`. `blinkStrike` measures between entity positions and has a separate `3.0`-`200.0` acquisition range (default `12.0`); it is mutually exclusive with `meleeAura`. Targets inside normal interaction reach use Minecraft's regular controller attack without position packets. Remote attacks advance the server-side position in configurable collision-checked steps, attack from `2.5` blocks away by default, and retrace the accepted path without moving the local camera. Candidate positions must also satisfy vanilla 1.12.2's entity-position distance and entity-eye visibility envelope, so a low or obstructed candidate that the server would reject is skipped in favor of another valid height or angle. Before sending, the complete outward, optional critical, and return movement burst is checked against vanilla's same-tick movement budget with one normal client movement packet reserved. The final outward position packet also carries the remote yaw and pitch, preventing a redundant movement packet from invalidating otherwise safe distant strikes. Equivalent same-height routes are removed before collision sampling. An unsafe critical burst is omitted; an unsafe ordinary round trip is not sent. Expensive planning is retained across ticks and globally capped at 12 target-plan checks, 16 candidate checks, and 96 collision samples per tick. Multi-target scanning still considers up to 50 entities, but only one remote packet excursion is executed per attack cycle to avoid correction storms and screen flashing. Controlled flight is accepted as an airborne origin. Blink reachability evidence is bound to the player's current feet cell and Flight state, so moving or changing Flight immediately refreshes red/green target status and route planning. Both combat modules expose a selectable attack body point (`head`, `chest`, `legs`, or `feet`) for rotation and aiming behavior. Longer settings, modified servers, and anti-cheat plugins may behave differently, so extended-range hits remain experimental and are not guaranteed.
...
Flight exposes only Static and Vanilla modes; loading `flight.mode=hypixel` rewrites it to `static`. Horizontal/ascent speed remains independently configurable from descent speed, whose default is `0.35` and range is `0.0`-`1.0` blocks per tick. Static applies current input before the same tick's vanilla movement and temporarily suppresses vanilla acceleration. Vanilla enables its flight capability during the same input event and compensates the later vanilla Sneak subtraction, so guarded descent reaches the configured motion before travel and converges to the measured surface distance. High-altitude disable still retains landing control until real ground contact or a recognized fall-safe state. Client fall accumulation is cleared locally, but a Survival server may still apply fall damage from distance it already accepted when real contact occurs.
...
The release artifact is `build/libs/voris-hub-1.10.146.jar`. `verifyRelease` runs unit tests and checks final JAR metadata, required classes, and Java 8 bytecode.
