...
- `blinkStrike`: experimental extended-range attack that sends a collision-checked sequence of temporary position packets, attacks near the target, then returns along a validated route.
- `flight`: WWE-style Static and Vanilla movement modes with independent horizontal/ascent and descent speeds, current-tick input ownership, and server-facing guarded landing behavior. Legacy Hypixel configs migrate to Static.
...
`meleeAura` measures its range from the player's eyes to the nearest point of a target's hitbox. Its default is `3.0` blocks and its configurable maximum is `6.0`. `blinkStrike` measures between entity positions and has a separate `3.0`-`200.0` acquisition range (default `12.0`); it is mutually exclusive with `meleeAura`. Targets inside normal interaction reach use Minecraft's regular controller attack without position packets. Remote attacks advance the server-side position in configurable collision-checked steps, attack from `2.5` blocks away by default, and return without moving the local camera. Candidate positions must also satisfy vanilla 1.12.2's entity-position distance and entity-eye visibility envelope. Before sending, the complete outward, optional critical, return, and airborne-release burst is checked against vanilla's same-tick movement budget from the tick-start position, with one normal client movement packet reserved. The final outward position packet also carries the remote yaw and pitch. A fully collision-checked direct route returns directly to its origin, avoiding a rejected sixth movement packet after ordinary ground or flight movement; dogleg routes retain their reverse path. Equivalent same-height routes are removed before collision sampling. Expensive planning remains capped at 12 target-plan checks, 16 candidate checks, and 96 collision samples per tick. Small player movement no longer discards pending work: absolute route evidence is retained, then a path rebuilt from the current origin is rechecked inside the same collision budget. A Flight-state change still discards incompatible pending evidence, while feet-cell changes immediately refresh red/green reachability caches. Controlled flight is accepted as an airborne origin, and Vanilla Flight transport restores the server to airborne state after the strike. Longer settings, modified servers, and anti-cheat plugins may behave differently, so extended-range hits remain experimental and are not guaranteed.

Target and ore visualization both default to `150` blocks and can be configured up to `500`. Ore visualization can only inspect chunks currently loaded by the client. Each vanilla ore has an independent switch and RGB outline color, plus one shared `0.0`-`1.0` outline brightness multiplier. Adjacent ore blocks are rendered as a shared outer wireframe so internal nine-grid style lines are skipped. Both visualizers can show compact nearby-count HUD items, sharing the same selectable screen corner. Ore scanning is chunk-cached, distance-culled, and reused by auto mining to avoid repeated large cube scans.

Auto mining uses `pathRange` for both target acquisition and route planning. When it starts without an enabled ore marker inside that range, the shared cache temporarily uses the higher discovery budget and scan waves let unfinished chunks yield to unstarted chunks; removing the final in-range marker restores that warm-up automatically. Each connected vein receives nearest-first invisible sequence labels when acquired, then keeps that order so adjacent ore cannot steal the active target. Before planning or continuing a route, any ore inside vanilla reach and actually hit by a center or exposed-face ray sample is mined directly, including partially exposed and overhead ore. Only destinations that were actually searched and found unreachable receive a retry cooldown; stale vein locks are released after the player leaves their candidate range. A vein label remains owned while a mined block is inside the three-tick server-confirmation window or its chunk is temporarily unavailable, so client prediction and short unloads cannot reorder competing adjacent targets before confirmation or rollback. Normal scan-marker updates are coalesced until the next four-tick candidate-cache boundary while an empty path retry is sleeping, preventing repeated candidate-heap rebuilds. A changed player feet cell, manual takeover, or screen pause cancels that old retry and invalidates its candidate snapshot, so returning near an ore does not inherit up to 20 ticks of idle time. Runtime obstacle failures now cool down the specific blocking cell and replan from the current position; target ore is cooled only after a completed stand position still cannot hit it or movement makes no progress for 30 ticks. When any target or obstacle cooldown expires, the retry delay, path-candidate snapshot, cached A* costs and nearest-candidate snapshot are refreshed together so the newly available route is retried in the same tick. The nearest-96 query also caches per-ore eligibility, prunes chunks whose nearest possible block center cannot improve the result, and avoids allocating rejected candidates. Ore scan slices reuse the existing per-type position index instead of rebuilding a temporary index for every partial chunk scan, and ore recognition retains identity-based positive and negative caches for encountered Block instances. Manual movement immediately cancels the old route and block-breaking action while preserving the player's own motion. The optional `辅助垫方块` setting is off by default; when enabled, it stays bound to the original player column, clicks the supporting face center, waits for server placement confirmation, then revalidates and mines the overhead ore. Each ore type has its own target count slider, where `0` means unlimited.
Route-owned obstacle clearing is cancelled before either tick phase can continue damage when its loaded ore owner disappears or changes type; temporarily unloaded ore chunks remain conservatively owned.
Replaceable route cells are treated as passable only when their actual collision shape does not intrude into the player's route column. Colliding layers such as two-layer snow are classified as clearable obstacles, keeping A* planning and runtime movement in agreement without changing collision-free air, water, or one-layer snow behavior.
Scaffold assist performs a scoped, server-synchronized sneak-place action, so workbenches, chests and other interactive supports do not consume the right-click and open a GUI. The player's original sneak and inventory state are restored after every attempt, while placement remains confirmed from the world block state.
...
Flight exposes only Static and Vanilla modes; loading `flight.mode=hypixel` rewrites it to `static`. Horizontal/ascent speed remains independently configurable from descent speed, whose default is `0.35` and range is `0.0`-`1.0` blocks per tick. Static applies current input before the same tick's vanilla movement and temporarily suppresses vanilla acceleration. Vanilla enables its flight capability during the same input event and compensates the later vanilla Sneak subtraction, so guarded descent reaches the configured motion before travel and converges to the measured surface distance. High-altitude disable still retains landing control until real ground contact or a recognized fall-safe state. During controlled Vanilla descent, a `true` then `false` ground-state packet pair is sent after normal movement and Blink handling, resetting the fall distance accepted by a stock 1.12.2 server without leaving the player grounded. This prevents newly controlled descent from accumulating fatal landing damage; it cannot undo a dangerous server-side fall distance accumulated before Flight took control, and modified anti-cheat behavior may differ.
...
Unreachable-route validation keeps its first pass sliced, then rechecks the retained final state set atomically before assigning the 100-tick failure cooldown, so a route opened mid-validation is not classified from a mixed multi-tick snapshot.
...
The release artifact is `build/libs/voris-hub-1.10.147.jar`. `verifyRelease` runs unit tests and checks final JAR metadata, required classes, and Java 8 bytecode.
