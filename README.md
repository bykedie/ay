...
- Combat modules have independent target filters, camera rotation, multi-target limits, target boxes, and mod-entity selection. Click a module's key-binding row and press a key to bind it; Delete or Backspace clears the binding.
- `/qazr status`
- `/qazr toggle <module>`
- `/qazr reload`
...
The release artifact is `build/libs/qazr-legacy-1.4.0.jar`. `verifyRelease` runs unit tests and checks final JAR metadata, required classes, and Java 8 bytecode.
