...
Voris Hub 是一个 Minecraft Forge 1.12.2 客户端工具模组，当前版本为 `1.10.141`。它不是 Meteor addon，也不是高版本 Fabric/Forge 项目；所有功能都基于 Forge 1.12.2、Java 8 和 1.12.2 的 MCP stable 39 映射实现。
...
- 当前发行 JAR: `build/libs/voris-hub-1.10.141.jar`
...
- 自动挖矿 1.10.112 补充：矿物和普通障碍都读取 Forge 1.12.2 `onPlayerDamageBlock` 的真实返回值；控制器拒绝破坏时不会挥手、累计尝试或登记完成确认，而会进入已有的目标/障碍冷却并重新规划。
- 自动挖矿 1.10.113 补充：矿物或普通障碍续挖时若自动选镐切换为会让 Forge 重置破坏进度的工具，会同步重置原版控制器、尝试次数、预算和截止时间；同一把可损耗工具仅耐久变化时不中断进度。
- 自动挖矿 1.10.114 补充：上一方块刚挖完后的原版命中间隔不再被误算为下一方块已经开始破坏；等待期间保留同一目标，但不挥手、不消耗尝试预算，也不登记待完成状态。
- 自动挖矿 1.10.115 补充：原版命中间隔等待现在每 tick 重试，不再被用户配置的挖掘延迟额外拉长；只有真正推进矿物或障碍破坏后才应用配置延迟。
- 自动挖矿 1.10.116 补充：在 Forge `ClientTickEvent.START` 阶段临时维持原版攻击键，使硬方块能跨 tick 连续累计破坏进度；`END` 阶段恢复玩家真实按键状态。挖掘延迟现在只在方块确认挖完后生效，不再打断同一方块的持续破坏。
- 自动挖矿 1.10.117 补充：继续破坏同一矿物或障碍时会重新读取当前相对硬度。若中途进水、离地或受到挖掘减速，尝试预算和截止时间只会扩展不会缩短，且不会重置已累计的原版破坏进度。
- 自动挖矿 1.10.118 补充：有效矿物或清障目标会在 `ClientTickEvent.START` 的原版输入处理前恢复最佳挖掘工具。自动近战或闪现攻击在上一 tick 切换武器后，不会再让下一次持续挖掘拿错工具或重置进度。
- 自动挖矿 1.10.119 补充：START 阶段只有在活动目标仍有效、仍在原版触及距离内，且七点射线能精确命中该方块时才会切换到挖掘工具，避免无效目标与战斗模块争夺手持槽。
- 自动挖矿 1.10.120 补充：游戏窗口失去焦点时会与打开 GUI 一样立即停止当前破坏、清障和路线动量，恢复真实攻击键状态。回到游戏后会重新选择目标，不再在后台消耗无效破坏预算。
- 自动挖矿 1.10.121 补充：到达挖矿终点后会先验证矿物是否已在原版触及距离内并被真实射线命中；低矮顶板不再让本可直接挖掘的矿物先进入站位否决与重规划。
- 自动挖矿 1.10.122 补充：任意带矿脉标签的方块确认挖完后都会按玩家当前位置重排剩余标签，包括为真正路线目标让路的同矿脉方块，避免旧距离顺序让近矿被上方或远处矿抢占。
- 自动挖矿 1.10.123 补充：终点暴露检测会逐条验证七点射线命中的遮挡；前面的命中若处于冷却、超距、站位不稳定或不可破坏，会继续检查后续采样，不再立刻否决当前站位。
- 自动挖矿 1.10.124 补充：普通候选发现继续使用中心与六个面中心的七点快速采样；已经锁定的矿物、路线终点、活动清障和辅助垫块目标在七点未命中后追加八个角点，能够继续处理仅露出边角的尾矿。
- 自动挖矿 1.10.125 补充：矿物射线命中后会保存实际暴露点，并在持续破坏时朝该面点或角点瞄准；不再识别出边角矿后又把视角转回被遮挡的方块中心。
- 自动挖矿 1.10.126 补充：锁定目标被多个同矿脉方块遮挡时，会先按隐形标签选择顺序更靠前的 blocker；标签相同再选择离眼睛更近的实际命中点，不再由射线遍历顺序抢目标。
- 自动挖矿 1.10.127 补充：START 阶段转向暴露点后会预刷新并复核原版准星；只有原版准星也精确命中活动方块时才临时维持攻击键，避免边角遮挡或实体挡线让原版切到其他目标并逐 tick 重置硬方块破坏进度。
- 自动挖矿 1.10.128 补充：分片寻路会在候选缓存真正刷新时核对未处理前沿；新发现的更近矿物若排到当前旧候选之前，会重建路径快照并及时参与选择，已处理批次末尾新增的低优先级远矿不会清空既有 A* 进度。
- 自动挖矿 1.10.129 补充：持续破坏的首个暴露点若被原版准星判为遮挡方块或实体，会继续有界检查其余面中心与角点；找到第一个原版也能精确命中的点后再维持攻击键，避免同一矿物仍有可用面点却反复中断。
- 自动挖矿 1.10.130 补充：横向下降路线会检查落点上方第二格的角色扫掠净空，并让 A* 代价、成功路线复验和预清障走廊共用同一判定，避免规划阶段放行但运行阶段撞头后反复重规划。
- 自动挖矿 1.10.131 补充：寻路失败冷却会绑定产生失败证据时的玩家脚格；玩家移动到新脚格后立即释放旧起点的目标冷却并刷新候选，避免路线条件已经改变却仍等待满 100 tick。破坏、清障、垫方块和具体站位失败冷却保持独立。
- 自动挖矿 1.10.132 补充：路线清障的 15 点射线会逐点验证通道归属、障碍冷却、原版触及距离、矿物站位和普通方块可破坏性；早期不可用命中不再截断后续可用面点，减少清障目标闪烁和无效重规划。
- 自动挖矿 1.10.133 补充：A* 搜索即使已重启过一次，耗尽后仍会完成两轮失败快照复验；若成功或失败复验再次发现世界变化，则延后当前尝试而不把矿物误记为不可达，避免错误进入 100 tick 路线冷却。
- 自动挖矿 1.10.134 补充：玩家进食、拉弓、举盾或使用其他物品时，START 与 END 阶段会共用输入门控，暂停自动切槽、转向、移动、垫块和破坏；当前工作的截止时间同步冻结，松开使用键后从原状态继续。
- 自动挖矿 1.10.140 补充：路线清障会在 START 与 END 阶段继续破坏前复核其已加载的矿物所有者；矿物消失或变成其他矿种时立即释放清障和路线，目标区块暂未加载时不会误取消。
- 自动挖矿 1.10.141 补充：不可达路线的第一遍状态复核继续分片，最终一遍会在同一 END tick 内原子复核最多 128 个保留状态，避免通道中途打开后仍由跨 tick 混合快照错误进入 100 tick 冷却。
...
- `blinkStrike`，中文名 `闪现攻击`：实验性扩展距离攻击。普通攻击距离内的可见目标直接使用原版控制器攻击，不发送位置包；远距离目标会发送经过碰撞检查的位置包序列，临近目标后攻击并沿路径返回。每个攻击周期最多执行一次远程位置往返。候选站位会同时复核原版 1.12.2 服务端使用的实体位置距离与目标眼睛可见性包络，服务端必然拒绝的低位或遮挡候选会继续尝试其他高度和角度。完整外出、旋转、可选暴击和回程移动包会在发送前按原版同 tick 移动预算预检；不安全的暴击附加包会省略，不安全的普通往返不会发送。红/绿可达缓存绑定当前脚格和 Flight 状态，移动或切换飞行会立即重新规划，不再复用旧高度的失败结果。默认搜索距离 `12.0`，最大 `200.0`。这不是服务端绕过保证，仍受服务端距离、移动和反作弊检查影响。
- `flight`，中文名 `飞行`：战斗分类中的 WWE 风格移动，支持静态、原版和 Hypixel 三种模式以及速度参数。潜行单独按下时进行受控下降，Jump+Sneak 保持中性；下降最高为 `1.0` 格/tick，并在接近地面时按真实碰撞距离收敛。Hypixel 模式在悬空时保留真实的未接地状态，不再主动触发一次半空落地结算。高空关闭模块后会继续控制下降，直到真实落地、水/岩浆、梯子、鞘翅或服务器允许飞行为止。客户端清零 `fallDistance` 不能清除生存服务器已经累计的摔落距离，因此不承诺在禁止飞行的服务器上绝对免伤。
...
截至本文档编写时，项目主功能状态为 `Voris Hub 1.10.141`：

- 构建产物名：`voris-hub-1.10.141.jar`
- 面板入口：`·`（Esc 下方的反引号键）
- 模块快捷键：默认未绑定，由用户自行绑定
- 目标可视化：默认 `150`，最大 `500`
...
- 飞行：支持 WWE 静态/原版/Hypixel 模式、速度参数、潜行受控着陆和高空关闭后的落地接管
