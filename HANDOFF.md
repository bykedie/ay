...
Voris Hub 是一个 Minecraft Forge 1.12.2 客户端工具模组，当前版本为 `1.10.146`。它不是 Meteor addon，也不是高版本 Fabric/Forge 项目；所有功能都基于 Forge 1.12.2、Java 8 和 1.12.2 的 MCP stable 39 映射实现。
...
- 当前发行 JAR: `build/libs/voris-hub-1.10.146.jar`
...
- `blinkStrike`，中文名 `闪现攻击`：实验性扩展距离攻击。普通攻击距离内的可见目标直接使用原版控制器攻击，不发送位置包；远距离目标会发送经过碰撞检查的位置包序列，临近目标后攻击并沿路径返回。每个攻击周期最多执行一次远程位置往返。候选站位会同时复核原版 1.12.2 服务端使用的实体位置距离与目标眼睛可见性包络，服务端必然拒绝的低位或遮挡候选会继续尝试其他高度和角度。完整外出、可选暴击和回程移动包会在发送前按原版同 tick 移动预算预检；最终外出位置包同时携带远端 yaw/pitch，避免额外旋转移动包让本可接受的较远往返被第六包阈值拒绝。同高度下与直线完全等价的狗腿路线会在碰撞采样前去重。不安全的暴击附加包会省略，不安全的普通往返不会发送。昂贵规划会跨 tick 保留，单 tick 全局上限仍为 12 个目标计划、16 个候选和 96 个碰撞样本；未完成搜索不会写入不可达缓存。红/绿可达缓存绑定当前脚格和 Flight 状态，移动或切换飞行会立即重新规划，不再复用旧高度的失败结果。默认搜索距离 `12.0`，最大 `200.0`。这不是服务端绕过保证，仍受服务端距离、移动和反作弊检查影响。
- `flight`，中文名 `飞行`：战斗分类中的 WWE 风格移动，仅提供静态和原版两种模式；旧配置 `flight.mode=hypixel` 会在加载时迁移并写回 `static`。水平/上升速度与下降速度独立，下降速度默认 `0.35`、范围 `0.0`-`1.0` 格/tick。静态模式在本 tick 输入刷新后、vanilla 移动前写入向量并临时屏蔽额外加速度，因此零速不漂移、配置速度不叠加、松键和反向无一拍延迟。原版模式也在 `InputUpdateEvent` 内先开启飞行能力，并预补偿随后原版执行的 `motionY -= flySpeed * 3`，使潜行下降在本 tick travel 前得到配置速度；Jump+Sneak 保持中性，接近地面时按真实碰撞距离收敛。高空关闭模块后会继续控制下降，直到真实落地、水/岩浆、梯子、鞘翅或服务器允许飞行为止。客户端清零 `fallDistance` 不能清除生存服务器已经累计的摔落距离，因此不承诺在禁止飞行的服务器上绝对免伤。
...
截至本文档编写时，项目主功能状态为 `Voris Hub 1.10.146`：

- 构建产物名：`voris-hub-1.10.146.jar`
- 面板入口：`·`（Esc 下方的反引号键）
- 模块快捷键：默认未绑定，由用户自行绑定
- 目标可视化：默认 `150`，最大 `500`
...
- 飞行：支持 WWE 静态/原版模式、旧 Hypixel 配置迁移、独立水平/上升与下降速度、原版本 tick 受控下降、潜行着陆和高空关闭后的落地接管
- 自动近战和闪现攻击最大目标数：`50`，并支持攻击部位选择；Blink 昂贵规划按 12/16/96 全局预算跨 tick 完成，最终外出位置与旋转合包并去除等价路线
- 用户可见品牌：`Voris Hub`
