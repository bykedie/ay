...
Voris Hub 是一个 Minecraft Forge 1.12.2 客户端工具模组，当前版本为 `1.10.137`。它不是 Meteor addon，也不是高版本 Fabric/Forge 项目；所有功能都基于 Forge 1.12.2、Java 8 和 1.12.2 的 MCP stable 39 映射实现。
...
- 当前发行 JAR: `build/libs/voris-hub-1.10.137.jar`
...
- `blinkStrike`，中文名 `闪现攻击`：实验性扩展距离攻击。普通攻击距离内的可见目标直接使用原版控制器攻击，不发送位置包；远距离目标会发送经过碰撞检查的位置包序列，临近目标后攻击并沿路径返回。每个攻击周期最多执行一次远程位置往返。红/绿可达缓存绑定当前脚格和 Flight 状态，移动或切换飞行会立即重新规划，不再复用旧高度的失败结果。默认搜索距离 `12.0`，最大 `200.0`。这不是服务端绕过保证，仍受服务端距离、移动和反作弊检查影响。
...
截至本文档编写时，项目主功能状态为 `Voris Hub 1.10.137`：

- 构建产物名：`voris-hub-1.10.137.jar`
- 面板入口：`·`（Esc 下方的反引号键）
- 模块快捷键：默认未绑定，由用户自行绑定
- 目标可视化：默认 `150`，最大 `500`
