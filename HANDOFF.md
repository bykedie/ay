...
Voris Hub 是一个 Minecraft Forge 1.12.2 客户端工具模组，当前版本为 `1.10.98`。它不是 Meteor addon，也不是高版本 Fabric/Forge 项目；所有功能都基于 Forge 1.12.2、Java 8 和 1.12.2 的 MCP stable 39 映射实现。
...
- 当前发行 JAR: `build/libs/voris-hub-1.10.98.jar`
...
- 自动挖矿 1.10.98 补充：若到达后的终点站位已失去稳定相对位置、脚位/头位净空或支撑条件，只排除该矿物的当前站位并立即重规划其他站位，避免 A* 反复选择同一个不可工作的脚位。若 A* 选中的挖掘站位就是玩家当前脚位，零节点路线会在同一 tick 进入终点复核，不再固定等待 3 tick。
...
截至本文档编写时，项目主功能状态为 `Voris Hub 1.10.98`：

- 构建产物名：`voris-hub-1.10.98.jar`
- 面板入口：`·`（Esc 下方的反引号键）
- 模块快捷键：默认未绑定，由用户自行绑定
- 目标可视化：默认 `150`，最大 `500`
