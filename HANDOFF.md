...
Voris Hub 是一个 Minecraft Forge 1.12.2 客户端工具模组，当前版本为 `1.10.144`。它不是 Meteor addon，也不是高版本 Fabric/Forge 项目；所有功能都基于 Forge 1.12.2、Java 8 和 1.12.2 的 MCP stable 39 映射实现。
...
- 当前发行 JAR: `build/libs/voris-hub-1.10.144.jar`
...
- `flight`，中文名 `飞行`：战斗分类中的 WWE 风格移动，支持静态、原版和 Hypixel 三种模式以及速度参数。静态模式在本 tick 输入刷新后、vanilla 移动前写入向量并临时屏蔽额外加速度，因此零速不漂移、配置速度不叠加、松键和反向无一拍延迟。潜行单独按下时进行受控下降，Jump+Sneak 保持中性；下降最高为 `1.0` 格/tick，并在接近地面时按真实碰撞距离收敛。Hypixel 模式在悬空时保留真实的未接地状态，不再主动触发一次半空落地结算。高空关闭模块后会继续控制下降，直到真实落地、水/岩浆、梯子、鞘翅或服务器允许飞行为止。客户端清零 `fallDistance` 不能清除生存服务器已经累计的摔落距离，因此不承诺在禁止飞行的服务器上绝对免伤。
...
截至本文档编写时，项目主功能状态为 `Voris Hub 1.10.144`：

- 构建产物名：`voris-hub-1.10.144.jar`
- 面板入口：`·`（Esc 下方的反引号键）
- 模块快捷键：默认未绑定，由用户自行绑定
- 目标可视化：默认 `150`，最大 `500`
