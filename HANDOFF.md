...
Voris Hub 是一个 Minecraft Forge 1.12.2 客户端工具模组，当前版本为 `1.10.147`。它不是 Meteor addon，也不是高版本 Fabric/Forge 项目；所有功能都基于 Forge 1.12.2、Java 8 和 1.12.2 的 MCP stable 39 映射实现。
...
- 当前发行 JAR: `build/libs/voris-hub-1.10.147.jar`
...
- `src/main/java/com/qazr/legacy/module/FlightController.java`: 战斗分类里的 WWE 风格飞行模块，含静态/原版模式、独立下降速度和潜行安全落地保护。
...
- 自动挖矿 1.10.142 补充：可替换材质只有在真实碰撞形状不侵入人物路线柱体时才算可通过；两层及以上雪层等碰撞型可替换方块会进入正常清障，空气、水和无碰撞的一层雪保持可通过。
- 自动挖矿 1.10.143 补充：辅助垫方块会在一次右键期间同步进入潜行放置，避免工作台、箱子等交互支撑打开 GUI；用户原潜行状态、快捷栏和临时背包换位都会在 `finally` 中恢复，落块仍以世界状态确认。
- 自动挖矿 1.10.145 补充：启用后若 `pathRange` 内尚无已启用矿种的 marker，共享矿物缓存会临时使用每 tick 最多 12 个扫描任务/16384 个方块的发现预算；未完成区块按 scan wave 让位给未开始区块。范围内最后一个 marker 被移除后会自动恢复该预热，远处同矿种或未启用矿种 marker 不会提前结束预热。
- `oreVisualizer`，中文名 `矿物可视化`：扫描客户端已加载区块，绘制原版矿石方框。默认距离 `150`，最大 `500`。每种矿石有独立开关和颜色，另有共享的 `0.0`-`1.0` RGB 亮度倍率。相邻同类矿石会合并成外边界线框，九宫格中心和内部边线不再绘制。扫描结果被缓存并提供给自动挖矿，渲染与计数会先做区块级距离裁剪。
...
- `blinkStrike`，中文名 `闪现攻击`：实验性扩展距离攻击。普通攻击距离内的可见目标直接使用原版控制器攻击，不发送位置包；远距离目标会发送经过碰撞检查的位置包序列，临近目标后攻击并返回。完整外出、可选暴击、回程和悬空恢复移动包会从本 tick 服务端起点按原版移动预算预检；最终外出位置包同时携带远端 yaw/pitch。已经完整碰撞复核的直线路径直接返回原点，避免玩家本 tick 已经地面移动或飞行后，逆序回程首包被原版第六包阈值拒绝；狗腿路线仍沿已验证路径逆序返回。昂贵规划跨 tick 保留，单 tick 全局上限仍为 12 个目标计划、16 个候选和 96 个碰撞样本；玩家小幅移动不会再清空未完成规划，而是在完成前用当前起点重建并在同一 96 样本预算内复核。脚格变化仍立即刷新红/绿可达缓存，Flight 状态切换会丢弃不兼容的未完成证据。默认搜索距离 `12.0`，最大 `200.0`。这不是服务端绕过保证，仍受服务端距离、移动和反作弊检查影响。
- `flight`，中文名 `飞行`：战斗分类中的 WWE 风格移动，仅提供静态和原版两种模式；旧配置 `flight.mode=hypixel` 会在加载时迁移并写回 `static`。水平/上升速度与下降速度独立，下降速度默认 `0.35`、范围 `0.0`-`1.0` 格/tick。静态模式保持原有本 tick 输入所有权。原版模式在 `InputUpdateEvent` 内先开启飞行能力，并预补偿随后原版执行的潜行扣速，使下降在本 tick travel 前得到配置速度；接近地面时按真实碰撞距离收敛。高空关闭模块后会继续控制下降，直到真实落地或进入认可的安全状态。受控原版下降会在普通移动和 Blink 之后发送 `onGround=true`、再发送 `onGround=false`，按原版服务端真实 `handleFalling` 规则逐 tick 清除新累计的摔落距离且不把服务端状态留在落地；Vanilla Flight 中的 Blink 位移也携带受控落地状态并在返程后恢复悬空。该策略不能抹除 Flight 接管前已经积累到危险值的服务端摔落距离，第三方反作弊也可能采用不同判定。
...
- `flight.mode` / `flight.speed` / `flight.descentSpeed`
- `autoGg.messages`
- `autoReply.messages`
...
- `blinkStrike` 是实验性功能。它通过位置包模拟短暂路径，不保证在所有服务器上生效。服务端距离检查、移动检查和反作弊插件可能拒绝或回滚。碰撞样本为避免卡顿而跨 tick 检查，完成前会重验目的地；若世界在已检查区段随后变化，一次计划仍可能失效，但不要恢复同步整条路线复验而重新引入主线程卡顿。
...
截至本文档编写时，项目主功能状态为 `Voris Hub 1.10.147`：

- 构建产物名：`voris-hub-1.10.147.jar`
- 面板入口：`·`（Esc 下方的反引号键）
- 模块快捷键：默认未绑定，由用户自行绑定
- 目标可视化：默认 `150`，最大 `500`
- 矿物可视化：默认 `150`，最大 `500`，亮度倍率 `0.0`-`1.0`，相邻同类矿石只绘制外边界
- 自动挖矿：支持原版距离真实射线直挖、位置与矿种绑定的服务端确认、挡路矿保留并绑定真正路线目标、矿石/普通障碍三 tick 稳定确认、普通障碍七点射线续挖、区块暂不可用时保留已观察到的矿石缺失证据、待确认矿和暂未加载矿的隐形标签在确认或回滚前保持原顺序、确认期间暂停目标切换与路线动量、确认回滚无冷却重规划、确认队列保护当前路线、取消工作即时释放未进入缺失确认的配额预留、确认挖完后按当前位置重排的稳定矿脉标签、大矿脉尾部轮转直挖、标签/冷却/配额变化即时刷新候选、目标冷却只延长不缩短且任一目标/障碍/终点站位冷却到期会原子清零重试等待并刷新路径快照、范围内无可用 marker 时临时提升为每 tick 最多 16384 个方块并按 scan wave 公平发现，找到范围内已启用矿种 marker 后恢复 4096，最后 marker 移除后重新预热、扫描期间同脚位 marker 变化合并到每四 tick 候选缓存边界，换格、手动接管或界面暂停会立即取消旧空路径等待并刷新候选、最近 96 个候选查询按矿种缓存资格并用区块/单矿距离下界剪枝、运行期障碍失败只冷却具体障碍并重规划，终点无法命中时先按矿物与脚位排除单个站位，只有所有站位搜索失败或连续 30 tick 无进展才冷却目标、A* 与成功/失败路线分片复核、首条路线后 4 tick 有界比较、同批失败目标独立冷却、下降成本去重、低碰撞支撑面平滑行走、路线走廊缓存、手动接管立即清理、过期标签释放、带服务端确认的辅助垫方块、矿种预设、每矿种预定数量、寻路范围和路线可视化
- 自动搭路：支持快捷栏/背包方块临时换位放置、跳跃/下落补桥、前探距离、下探高度、放置间隔和防卡脚参数
- 飞行：支持 WWE 静态/原版模式、旧 Hypixel 配置迁移、独立水平/上升与下降速度、原版本 tick 受控下降、服务端摔落累计重置、潜行着陆和高空关闭后的落地接管
- 自动近战和闪现攻击最大目标数：`50`，并支持攻击部位选择；Blink 昂贵规划按 12/16/96 全局预算跨 tick 完成，小幅移动后复核重建路径，直线安全回程按服务端 tick 起点预算预检
- 用户可见品牌：`Voris Hub`
- 内部兼容品牌：`qazrlegacy` 保留

接手时请以当前仓库 `main` 分支源码为准。如果文档和代码冲突，以代码、测试和实际构建结果为准，并及时更新本文档。

本轮按用户要求只创建本地提交，不执行 `git push`；`reports/` 下既有未跟踪审查报告继续保留且不纳入提交。
