...
Voris Hub 是一个 Minecraft Forge 1.12.2 客户端工具模组，当前版本为 `1.10.101`。它不是 Meteor addon，也不是高版本 Fabric/Forge 项目；所有功能都基于 Forge 1.12.2、Java 8 和 1.12.2 的 MCP stable 39 映射实现。
...
- 当前发行 JAR: `build/libs/voris-hub-1.10.101.jar`
...
- 自动挖矿 1.10.101 补充：非空路线确认到达最后一个站位并停止移动后，会在同一 tick 进入终点工作区、挖掘面和射线复核，不再额外空等一个 tick；零节点路线保持同样的即时交接。共享矿物缓存仍会等待世界和玩家对象都就绪后再消费扫描任务。
...
截至本文档编写时，项目主功能状态为 `Voris Hub 1.10.101`：

- 构建产物名：`voris-hub-1.10.101.jar`
- 面板入口：`·`（Esc 下方的反引号键）
- 模块快捷键：默认未绑定，由用户自行绑定
- 目标可视化：默认 `150`，最大 `500`
