...
Voris Hub 是一个 Minecraft Forge 1.12.2 客户端工具模组，当前版本为 `1.10.103`。它不是 Meteor addon，也不是高版本 Fabric/Forge 项目；所有功能都基于 Forge 1.12.2、Java 8 和 1.12.2 的 MCP stable 39 映射实现。
...
- 当前发行 JAR: `build/libs/voris-hub-1.10.103.jar`
...
- 自动挖矿 1.10.103 补充：活动破坏目标切换到不同位置或矿种时，会释放旧目标尚未观察到缺失的配额预留，避免有限挖掘数量被旧目标虚占至确认超时；如果旧方块随后确实消失，缺失确认会重新预留并正常计数。路线清障链仍会跨前置矿保留真正路线目标。
...
截至本文档编写时，项目主功能状态为 `Voris Hub 1.10.103`：

- 构建产物名：`voris-hub-1.10.103.jar`
- 面板入口：`·`（Esc 下方的反引号键）
- 模块快捷键：默认未绑定，由用户自行绑定
- 目标可视化：默认 `150`，最大 `500`
