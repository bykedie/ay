...
Voris Hub 是一个 Minecraft Forge 1.12.2 客户端工具模组，当前版本为 `1.10.99`。它不是 Meteor addon，也不是高版本 Fabric/Forge 项目；所有功能都基于 Forge 1.12.2、Java 8 和 1.12.2 的 MCP stable 39 映射实现。
...
- 当前发行 JAR: `build/libs/voris-hub-1.10.99.jar`
...
- 自动挖矿 1.10.99 补充：终点前的挡路矿会在整个持续破坏周期内保留原路线所有权，后续挖掘 tick 不会把真正路线目标改成挡路矿或覆盖确认绑定。无效终点站位仍只按矿物与脚位排除并改试其他站位；零节点路线会在同一 tick 进入终点复核。
...
截至本文档编写时，项目主功能状态为 `Voris Hub 1.10.99`：

- 构建产物名：`voris-hub-1.10.99.jar`
- 面板入口：`·`（Esc 下方的反引号键）
- 模块快捷键：默认未绑定，由用户自行绑定
- 目标可视化：默认 `150`，最大 `500`
