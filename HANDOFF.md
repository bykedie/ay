...
Voris Hub 是一个 Minecraft Forge 1.12.2 客户端工具模组，当前版本为 `1.10.82`。它不是 Meteor addon，也不是高版本 Fabric/Forge 项目；所有功能都基于 Forge 1.12.2、Java 8 和 1.12.2 的 MCP stable 39 映射实现。
...
- 当前发行 JAR: `build/libs/voris-hub-1.10.82.jar`
...
截至本文档编写时，项目主功能状态为 `Voris Hub 1.10.82`：

- 构建产物名：`voris-hub-1.10.82.jar`
- 面板入口：`·`（Esc 下方的反引号键）
- 模块快捷键：默认未绑定，由用户自行绑定
- 目标可视化：默认 `150`，最大 `500`
