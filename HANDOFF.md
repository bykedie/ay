...
Voris Hub 是一个 Minecraft Forge 1.12.2 客户端工具模组，当前版本为 `1.10.39`。它不是 Meteor addon，也不是高版本 Fabric/Forge 项目；所有功能都基于 Forge 1.12.2、Java 8 和 1.12.2 的 MCP stable 39 映射实现。
...
- 当前发行 JAR: `build/libs/voris-hub-1.10.39.jar`
...
- `blinkStrike`，中文名 `闪现攻击`：实验性扩展距离攻击。它会发送经过碰撞检查的位置包序列，临近目标后发送攻击包，再沿路径返回。飞行状态可作为安全空中起点，但空中包保持 `onGround=false`；检测到服务端位置纠正后只回正一次并停手 40 tick，同时目标显示为红色。默认搜索距离 `12.0`，最大 `200.0`。这不是服务端绕过保证，仍受服务端距离、移动和反作弊检查影响。
- `flight`，中文名 `飞行`：战斗分类中的 WWE 风格移动，支持静态、原版和 Hypixel 三种模式以及速度参数。保持飞行开启并按住潜行会限制下降速度、周期清理摔落累计，并在接近碰撞面时确认落地；接触地面后再关闭飞行。
...
截至本文档编写时，项目主功能状态为 `Voris Hub 1.10.39`：

- 构建产物名：`voris-hub-1.10.39.jar`
- 面板入口：`·`（Esc 下方的反引号键）
- 模块快捷键：默认未绑定，由用户自行绑定
- 目标可视化：默认 `150`，最大 `500`
