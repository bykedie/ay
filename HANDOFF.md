...
Voris Hub 是一个 Minecraft Forge 1.12.2 客户端工具模组，当前版本为 `1.10.100`。它不是 Meteor addon，也不是高版本 Fabric/Forge 项目；所有功能都基于 Forge 1.12.2、Java 8 和 1.12.2 的 MCP stable 39 映射实现。
...
- 当前发行 JAR: `build/libs/voris-hub-1.10.100.jar`
...
- 自动挖矿 1.10.100 补充：共享矿物缓存只在世界和玩家对象都已就绪后消费扫描任务；玩家尚未建立时由区块加载事件排入的任务会保留到后续 tick，避免进服初始化阶段因空玩家坐标中断扫描。终点挡路矿仍会在整个持续破坏周期内保留原路线所有权。
...
截至本文档编写时，项目主功能状态为 `Voris Hub 1.10.100`：

- 构建产物名：`voris-hub-1.10.100.jar`
- 面板入口：`·`（Esc 下方的反引号键）
- 模块快捷键：默认未绑定，由用户自行绑定
- 目标可视化：默认 `150`，最大 `500`
